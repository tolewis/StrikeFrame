...
function normalizeConfig(raw) {
  ...
  return {
    ...
    overlay: Object.assign({ leftOpacity: 0.78, midOpacity: 0.42, rightOpacity: 0.18, vignetteBottom: 0.30 }, raw.overlay || {}),
    text: Object.assign({
    ...
    <svg width="${width}" height="${height}" xmlns="http://www.w3.org/2000/svg">
      <defs>
        <linearGradient id="bg" x1="0" y1="0" x2="1" y2="0">
          <stop offset="0%" stop-color="rgba(5,14,24,${overlay.leftOpacity})"/>
          <stop offset="58%" stop-color="rgba(5,14,24,${overlay.midOpacity})"/>
          <stop offset="100%" stop-color="rgba(5,14,24,${overlay.rightOpacity})"/>
        </linearGradient>
...
function buildTextSvg(cfg) {
  const { width, height, text, layout } = cfg;
  ...
  return Buffer.from(`
    <svg width="${width}" height="${height}" xmlns="http://www.w3.org/2000/svg">
      <rect x="${layout.ctaRectX}" y="${layout.ctaRectY}" width="${layout.ctaWidth}" height="${layout.ctaHeight}" rx="29" fill="rgba(255,255,255,0.16)" stroke="rgba(255,255,255,0.28)" />
      <text x="${layout.leftX}" y="${layout.headlineY}" fill="#ffffff" font-size="${headlineFont}" font-family="Arial, Helvetica, sans-serif" font-weight="700">${headlineTspans}</text>
      <text x="${layout.leftX}" y="${layout.subheadY}" fill="#e8eef2" font-size="${subheadFont}" font-family="Arial, Helvetica, sans-serif" font-weight="400">${subheadTspans}</text>
      <text x="${layout.ctaX}" y="${layout.ctaY}" fill="#ffffff" font-size="24" font-family="Arial, Helvetica, sans-serif" font-weight="700">${escapeXml(text.cta)}</text>
      <text x="${layout.leftX}" y="${layout.footerY}" fill="#d6dde2" font-size="${footerFont}" font-family="Arial, Helvetica, sans-serif" letter-spacing="3">${escapeXml(text.footer)}</text>
    </svg>`);
}

function buildCompositeSvg(cfg) {
  const { width, height, productComposite } = cfg;
  if (!productComposite.enabled) return null;
  const d = productComposite.circleDiameter;
  return Buffer.from(`
    <svg width="${width}" height="${height}" xmlns="http://www.w3.org/2000/svg">
      <circle cx="${productComposite.circleX + d / 2}" cy="${productComposite.circleY + d / 2 + 20}" r="${d / 2}" fill="rgba(0,0,0,${productComposite.shadowOpacity})" />
      <circle cx="${productComposite.circleX + d / 2}" cy="${productComposite.circleY + d / 2}" r="${d / 2}" fill="rgba(255,255,255,0.94)" />
      <rect x="${productComposite.badgeX}" y="${productComposite.badgeY}" width="170" height="42" rx="21" fill="rgba(255,255,255,0.16)" stroke="rgba(255,255,255,0.24)" />
      <text x="${productComposite.badgeX + 24}" y="${productComposite.badgeY + 28}" fill="#ffffff" font-size="20" font-family="Arial, Helvetica, sans-serif" font-weight="700">${escapeXml(productComposite.badgeText)}</text>
    </svg>`);
}
        ...
        <svg width="${cfg.width}" height="${cfg.height}" xmlns="http://www.w3.org/2000/svg">
          <defs>
            <linearGradient id="g" x1="0" y1="0" x2="1" y2="1">
              <stop offset="0%" stop-color="#0b2a40"/>
              <stop offset="100%" stop-color="#1f6b8f"/>
            </linearGradient>
          </defs>
