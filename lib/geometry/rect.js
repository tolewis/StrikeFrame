 ...
 * Skips parent-child relationships (elements sharing a dot-prefix are in
 * the same primitive, and 'layout' type elements contain their children).
 * Returns array of { a, b, intersection } for each real collision.
 */
function findCollisions(elements) {
  ...
  for (let i = 0; i < elements.length; i++) {
    for (let j = i + 1; j < elements.length; j++) {
      const a = elements[i];
      const b = elements[j];
      // Skip intra-primitive containment (e.g. comparisonPanel.table contains comparisonPanel.row.*)
      ...
      // Skip elements within the same primitive container
      const aParts = a.id.split('.');
      const bParts = b.id.split('.');
      if (aParts.length >= 2 && bParts.length >= 2 && aParts[0] === bParts[0]) {
        // Same primitive — skip if one is a layout container
