...
function scoreGeometry(layout) {
  ...
  const hardViolations = (layout.safeZones?.violations || []).filter(v => v.type === 'hard');
  const softViolations = (layout.safeZones?.violations || []).filter(v => v.type === 'soft');
  if (hardViolations.length > 0) {
    score.value -= Math.min(40, hardViolations.length * 15);
    score.findings.push({
      type: 'fail',
      rule: 'safe_zone_hard',
      message: `${hardViolations.length} element(s) violate hard safe zones`,
      targets: hardViolations.map(v => v.id),
      action: 'Move elements inside safe zone boundaries'
    });
  }
  if (softViolations.length > 0) {
    score.value -= Math.min(15, softViolations.length * 5);
    score.findings.push({
      type: 'warn',
      rule: 'safe_zone_soft',
      message: `${softViolations.length} element(s) violate soft safe zones`,
      targets: softViolations.map(v => v.id)
    });
  }
  ...
  // Occupancy — too sparse or too dense
  ...
  if (occ > 0.6) {
    score.value -= 10;
    score.findings.push({ type: 'warn', rule: 'canvas_dense', message: `Canvas ${Math.round(occ * 100)}% occupied — feels crowded` });
  }
...
function scoreReadability(layout) {
  ...
  // Mobile font size warnings
  const tooSmall = mobileWarnings.filter(w => w.canvasSize && w.mobileSize);
  if (tooSmall.length > 0) {
    // Only penalize real content elements, not empty placeholders (fontSize=1)
    const realSmall = tooSmall.filter(w => w.canvasSize > 10);
    if (realSmall.length > 0) {
      score.value -= Math.min(30, realSmall.length * 8);
      score.findings.push({
        type: 'warn',
...
function scoreSpacing(layout) {
  ...
  // Check vertical balance — content should use 35-90% of canvas height
  ...
  if (bbox && layout.canvas) {
    ...
    if (utilization < 0.30) {
      ...
      score.findings.push({
        ...
        message: `Content uses only ${Math.round(utilization * 100)}% of canvas height — feels sparse`,
        action: 'Spread elements vertically or add supporting content'
      });
    }
    if (utilization > 0.95) {
      score.value -= 10;
      score.findings.push({
        type: 'warn',
        rule: 'vertical_cramming',
        message: `Content fills ${Math.round(utilization * 100)}% of canvas height — no breathing room`,
        action: 'Reduce element count or font sizes'
      });
    }
  }
