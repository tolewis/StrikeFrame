...
function resolveQuoteLayout(quote, p, helpers, canvasWidth) {
  ...
  let bestWidth = Math.max(...lines.map(line => estimateTextWidth(line, fontSize)));
  ...
  solveForFont();
  const minQuoteSize = p.minQuoteSize || Math.max(54, fontSize - 10);
...
function resolveProofHero(cfg, helpers) {
  ...
  const starsRenderHeight = Math.round(starsSize * 1.2);
  const starsX = p.starsX || quoteLayout.quoteX + 130;
  const quoteToStarsGap = p.quoteToStarsGap || 26;
  const starsTop = Math.max((p.starsY ? p.starsY - starsSize : 0), quoteLayout.quoteTop + quoteLayout.quoteHeight + quoteToStarsGap);
  const starsY = starsTop + starsSize;

  const reviewX = p.reviewX || 118;
  const starsToReviewGap = p.starsToReviewGap || 42;
  const reviewY = Math.max(p.reviewY || 0, starsTop + starsRenderHeight + starsToReviewGap);
  const reviewWidth = p.reviewWidth || Math.round(cfg.width * 0.66);
  const reviewHeight = p.reviewHeight || 210;

  const ctaHeight = p.cta?.height || 74;
  const safeBottom = (cfg.constraintPolicy?.geometry?.minSafeZone || 40);
  const maxCtaTop = cfg.height - safeBottom - ctaHeight;
  ...
  const elements = [
    ...
    { id: 'proofHero.stars', type: 'text', rect: rect(starsX, starsTop, Math.round(starsSize * 4.2), starsRenderHeight) }
  ];
